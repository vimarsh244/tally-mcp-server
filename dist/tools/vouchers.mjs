...
import { cachedTable, columns, fail, guard, isoDate, ok, readOnly, targetCompany } from './shared.mjs';
...
async function voucherReport(report, guid, date, company) {
    const inputs = new Map([['fromDate', date], ['toDate', date], ['voucherGuid', tdlString(guid)]]);
    if (company)
...
export const voucherTools = ({ server, cache }) => {
    server.registerTool('daybook', {
        ...
        const all = renameObjectArrayProperties(Array.isArray(response.data) ? response.data : [], new Map([['party_ledger', 'party_name']]));
        ...
        return cachedTable(cache, columns(['guid', 'string'], ['date', 'date'], ['voucher_type', 'string'], ['voucher_number', 'string'], ['reference', 'string'], ['party_name', 'string'], ['amount', 'number'], ['narration', 'string'], ['is_cancelled', 'boolean'], ['is_optional', 'boolean']), page, { company: args.targetCompany, fromDate: args.fromDate, toDate: args.toDate }, {
    ...
    server.registerTool('voucher-get', {
        ...
        if (args.targetCompany)
            header.set('targetCompany', args.targetCompany);
        const dayResponse = await fetchReport('daybook', header);
        if (dayResponse.error)
            return fail(dayResponse.error);
        const rows = Array.isArray(dayResponse.data) ? dayResponse.data : [];
...
//# sourceMappingURL=vouchers.mjs.map