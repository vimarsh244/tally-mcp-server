...
import { cachedTable, columns, guard, isoDate, readOnly, targetCompany } from './shared.mjs';
export const statementTools = ({ server, cache }) => {
    server.registerTool('chart-of-accounts', {
        ...
        const rows = renameObjectArrayProperties(await queryCollection('Ledger', ['Name', 'Parent', '_PrimaryGroup', 'IsRevenue', 'IsDeemedPositive', 'AffectsGrossProfit', 'SortPosition'], new Map(), args.targetCompany), new Map([['Name', 'ledger_name'], ['Parent', 'group_name'], ['_PrimaryGroup', 'primary_group'], ['IsRevenue', 'bs_pl'], ['IsDeemedPositive', 'dr_cr'], ['AffectsGrossProfit', 'affects_gross_profit'], ['SortPosition', 'sort_position']]));
        return cachedTable(cache, columns(['ledger_name', 'string'], ['group_name', 'string'], ['primary_group', 'string'], ['bs_pl', 'boolean'], ['dr_cr', 'boolean'], ['affects_gross_profit', 'boolean'], ['sort_position', 'number']), rows, { company: args.targetCompany });
    }));
    server.registerTool('trial-balance', {
        ...
        const rows = renameObjectArrayProperties(await queryCollection('Ledger', ['Name', 'Parent', 'OpeningBalance', 'DebitTotals', 'CreditTotals', 'ClosingBalance'], filters, args.targetCompany, new Date(args.fromDate), new Date(args.toDate)), new Map([['Name', 'ledger_name'], ['Parent', 'group_name'], ['OpeningBalance', 'opening_balance'], ['DebitTotals', 'net_debit'], ['CreditTotals', 'net_credit'], ['ClosingBalance', 'closing_balance']]));
        return cachedTable(cache, columns(['ledger_name', 'string'], ['group_name', 'string'], ['opening_balance', 'amount'], ['net_debit', 'amount'], ['net_credit', 'amount'], ['closing_balance', 'amount']), rows, { company: args.targetCompany, fromDate: args.fromDate, toDate: args.toDate }, { checks: {
    ...
    server.registerTool('profit-loss', {
        ...
        const rows = [];
        const ledgers = renameObjectArrayProperties(await queryCollection('Ledger', ['Name', 'Parent', 'ClosingBalance'], new Map([['PL_Group', '$IsRevenue']]), args.targetCompany, from, to), new Map([['Name', 'ledger_name'], ['Parent', 'group_name'], ['ClosingBalance', 'closing_balance']]));
        ...
        rows.push(...ledgers);
        return cachedTable(cache, columns(['ledger_name', 'string'], ['group_name', 'string'], ['closing_balance', 'amount']), rows, { company: args.targetCompany, fromDate: args.fromDate, toDate: args.toDate }, { checks: { closingBalanceTotal: total(rows, 'closing_balance') } });
    }));
    server.registerTool('balance-sheet', {
        ...
        const from = new Date(args.fromDate);
        const to = new Date(args.toDate);
        const rows = renameObjectArrayProperties(await queryCollection('Ledger', ['Name', 'Parent', 'ClosingBalance'], new Map([['BS_Group', 'NOT $IsRevenue'], ['Excl_Stock', 'NOT $$IsGroupStock']]), args.targetCompany, from, to), new Map([['Name', 'ledger_name'], ['Parent', 'group_name'], ['ClosingBalance', 'closing_balance']]));
        ...
        for (const row of rows)
            if (row.ledger_name === PROFIT_AND_LOSS)
                row.group_name = '';
