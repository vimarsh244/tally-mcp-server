...
import { cachedTable, collectionNames, fail, guard, okText, readOnly, targetCompany } from './shared.mjs';
/** Cache column type for a collection field. */
const cacheType = (datatype) => {
    if (['amount', 'quantity', 'rate', 'number'].includes(datatype))
        return 'number';
    if (datatype === 'date' || datatype === 'boolean')
        return datatype;
    return 'string';
};
export const queryTools = ({ server, cache }) => {
    ...
    server.registerTool('query-collection', {
        ...
        const rows = await queryCollection(collection, requested, new Map(), args.targetCompany, args.fromDate ? new Date(args.fromDate) : undefined, args.toDate ? new Date(args.toDate) : undefined);
