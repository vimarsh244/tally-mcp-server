 ...
 */
export function allowedHosts(domain = config.domain, port = config.port) {
    const hosts = new Set();
    try {
...
export function isLocalDomain(domain = config.domain) {
...
export function assertPasswordIsSafe() {
    ...
    if (isLocalDomain() || config.allowDefaultPassword)
        return;
    throw new Error(`Refusing to start on ${config.domain} with the default password. `
