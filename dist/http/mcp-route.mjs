...
export function registerMcpRoutes(router, store, resolveRealm, options = {}) {
    const sessions = new Map();
    const hosts = options.allowedHosts ?? allowedHosts();
    const unauthorized = (res, realm, message) => {
    ...
    const sessionFor = (req, caller) => {
        ...
        if (!session)
            return undefined;
        if (session.clientId !== caller.clientId || session.realm.id !== caller.realm.id)
            return undefined;
        return session;
    };
