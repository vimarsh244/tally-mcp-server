...
function authorizationServerDocument(realm) {
    return {
        issuer: realm.issuer,
 ...
 */
export function registerWellKnownRoutes(app, resolveRealm) {
    const serve = (build) => (req, res) => {
        const realm = resolveRealm(req);
        if (!realm)
            return unknownRealm(res);
...
export function registerOAuthRoutes(router, store, publicDir, resolveRealm) {
...
function handleRefresh(req, res, store, clientId, realm) {
...
function respondWithTokens(res, store, clientId, realm) {
