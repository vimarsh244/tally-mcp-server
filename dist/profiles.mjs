...
export function defaultDataDir() {
    if (process.env.TALLY_MCP_DATA_DIR)
        return process.env.TALLY_MCP_DATA_DIR;
...
export function safeEqualsHex(a, b) {
...
export class ProfileStore {
    ...
    save() {
        ...
        const payload = { version: 1, profiles: [...this.profiles.values()] };
    ...
    update(id, patch) {
        ...
        if (!profile)
            throw new Error(`No profile named ${id}`);
        ...
        if (patch.windowsUser !== undefined)
            profile.windowsUser = patch.windowsUser;
        if (patch.tallyHost !== undefined)
            profile.tallyHost = patch.tallyHost || '127.0.0.1';
        if (patch.dataPath !== undefined)
            profile.dataPath = patch.dataPath;
        if (patch.blockWrite !== undefined)
            profile.blockWrite = patch.blockWrite;
        this.save();
 ...
 */
export function ensureAdminToken(dir = defaultDataDir()) {
