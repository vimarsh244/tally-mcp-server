...
export function collectionDefinition(collection) {
...
export async function queryCollection(targetCollection, lstFields, lstFilters, targetCompany, fromDate, toDate) {
    ...
    if (fromDate)
        args.set('fromDate', fromDate);
    if (toDate)
        args.set('toDate', toDate);
    args.set('collection', targetCollection);
    args.set('fields', queryFields);
    if (lstFilters && lstFilters.size > 0)
        args.set('filters', [...lstFilters].map(([name, expression]) => ({ name, expression })));
...
function convertField(raw, datatype) {
    if (datatype === 'boolean')
        return raw === 'Yes';
