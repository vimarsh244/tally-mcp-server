...
import { collectionNames, fail, guard, isoDate, ok, targetCompany, writes, type ToolResult, type ToolModule } from './shared.mjs';
...
const toAddressLines = (value: string | string[] | undefined): string[] | undefined => {
...
type LedgerMaster = z.infer<typeof ledgerMaster>;

/** Copies only the properties the caller actually supplied, so unset fields stay unchanged in Tally. */
function toLedgerPayload(master: LedgerMaster, booksBeginFrom: Date): Record<string, any> {
    const payload: Record<string, any> = {};
    if (master._name) payload._name = master._name;
    if (master.name) payload.name = master.name;
    if (master.parent) payload.parent = master.parent;
    if (master.openingBalance !== undefined) payload.openingBalance = master.openingBalance;
    ...
    if (master.contactDetails) payload.contactDetails = master.contactDetails;
    if (master.bankDetails) payload.bankDetails = master.bankDetails;
    ...
    if (master.isBillWise !== undefined) payload.isBillWise = master.isBillWise;
...
async function resolveCompany(name?: string): Promise<{ company: any } | { error: ToolResult }> {
...
async function pushMasters(template: string, masters: any[], company?: string): Promise<ToolResult> {
    const input = new Map<string, any>();
    input.set('masters', masters);
    if (company) input.set('targetCompany', company);
...
export const writeTools: ToolModule = ({ server }) => {
    ...
    server.registerTool('ledger-create-update', {
        ...
        return pushMasters('master-ledger',
            args.masters.map((master) => toLedgerPayload(master, resolved.company.BooksFrom)),
            args.targetCompany);
    }));
    ...
    server.registerTool('godown-create-update', {
        ...
        annotations: writes,
    }, guard(async (args) => pushMasters('master-godown',
        args.masters.map((master) => ({ ...master, address: toAddressLines(master.address) })),
        args.targetCompany)));
    ...
    server.registerTool('stock-item-create-update', {
        ...
        return pushMasters('master-stock-item',
            args.masters.map((master) => master.gstDetails
