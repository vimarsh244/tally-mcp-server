...
import { fail, guard, isoDate, ok, targetCompany, writes, type ToolResult, type ToolModule } from './shared.mjs';
...
function balanceOf(voucher: VoucherInput): number {
    let total = voucher.entries.reduce((sum, entry) => sum + entry.amount, 0);
    for (const item of voucher.inventoryEntries ?? [])
        if (item.ledgerName) total += item.amount;
    return round4(total);
}

/** Every ledger name a voucher refers to, so all of them can be checked in one query. */
function ledgerNamesOf(voucher: VoucherInput): string[] {
    const names = voucher.entries.map((entry) => entry.ledgerName);
    if (voucher.partyLedgerName) names.push(voucher.partyLedgerName);
    for (const item of voucher.inventoryEntries ?? [])
        if (item.ledgerName) names.push(item.ledgerName);
    return names;
}

/** Reports every problem at once, so a caller does not fix them one round trip at a time. */
function validate(voucher: VoucherInput, index: number, known: { ledgers: Set<string>; voucherTypes: Set<string>; stockItems: Set<string> }): string[] {
...
function toVoucherPayload(voucher: VoucherInput, action: 'Create' | 'Alter' | 'Cancel' | 'Delete'): Record<string, any> {
...
async function daybookOn(date: string, company?: string): Promise<any[]> {
...
const nameSet = async (collection: string, company?: string): Promise<Set<string>> =>
    new Set((await queryCollection(collection, ['Name'], new Map(), company)).map((row) => row.Name));

export const voucherWriteTools: ToolModule = ({ server }) => {
    ...
    server.registerTool('voucher-create-update', {
        ...
        const known = {
            ...
            stockItems: needsStock ? await nameSet('StockItem', args.targetCompany) : new Set<string>(),
        };
        ...
        if (args.targetCompany) input.set('targetCompany', args.targetCompany);
        ...
        for (const date of dates)
            for (const row of await daybookOn(date, args.targetCompany))
                if (vouchers.some((voucher) => voucher.date === date && (
                    (voucher.guid && voucher.guid === row.guid)
