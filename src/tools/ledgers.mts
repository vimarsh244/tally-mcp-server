...
import { cachedTable, collectionNames, columns, fail, guard, isoDate, ok, readOnly, targetCompany, type ToolModule } from './shared.mjs';

const MASTER_COLLECTIONS = ['group', 'ledger', 'vouchertype', 'unit', 'godown', 'stockgroup', 'stockitem', 'costcategory', 'costcentre', 'attendancetype', 'company', 'currency', 'gstin', 'gstclassification'] as const;

export const ledgerTools: ToolModule = ({ server, cache }) => {
    ...
    server.registerTool('bills-outstanding', {
        ...
        const rows = renameObjectArrayProperties(
            await queryCollection('Bill', ['BillDate', 'Name', 'ClosingBalance', 'Parent', '_OverDueDays'], filters, args.targetCompany, undefined, new Date(args.toDate)),
            new Map([['BillDate', 'bill_date'], ['Name', 'reference_number'], ['ClosingBalance', 'outstanding_amount'], ['Parent', 'party_name'], ['_OverDueDays', 'overdue_days']]));
...
export function openingFirst(data: any): any[] {
