 ...
 * nothing in it.
 */
export function tallyError(xml: string): string | undefined {
    if (!xml || !xml.trim())
    ...
    if (exception) return parseText(exception[1].trim()) || 'Tally reported an exception';

    const lineError = LINE_ERROR.exec(xml);
    if (lineError) return parseText(lineError[1].trim()) || 'Tally reported an error';
...
export function assertTallyResponse(xml: string): void {
 ...
 */
export function parseRows(xml: string): any[] {
...
export function parseDate(value: string): Date | null {
    if (/^\d\d\d\d-\d\d-\d\d$/.test(value)) return utility.Date.parse(value, 'yyyy-MM-dd');
    if (/^\d?\d-\w\w\w-\d\d\d\d$/.test(value)) return utility.Date.parse(value, 'd-MMM-yyyy');
    if (/^\d?\d-\w\w\w-\d\d$/.test(value)) return utility.Date.parse(value, 'd-MMM-yy');
    return null;
}
