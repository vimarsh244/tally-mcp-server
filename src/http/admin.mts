...
import { PROFILE_ID_PATTERN, safeEqualsHex, type ProfileStore } from '../profiles.mjs';
...
export interface AdminOptions {
    ...
    adminToken: string;
    /** Set false only in tests that drive the API from a non-loopback address. */
...
const isLoopback = (address: string | undefined): boolean =>
    typeof address === 'string' && LOOPBACK.has(address);

export interface ProbeResult {
    host: string;
    port: number;
    reachable: boolean;
    companies: { name: string; active: boolean }[];
    error?: string;
}

/** Asks one address whether a Tally is there, and which companies it has open. */
export async function probeTally(host: string, port: number, timeout = PROBE_TIMEOUT_MS): Promise<ProbeResult> {
    try {
        const rows = await runWithTallyTarget({ host, port, timeout },
            () => queryCollection('Company', ['Name', 'IsActiveCompany'], new Map()));
...
export function registerAdminRoutes(app: express.Express, publicDir: string, options: AdminOptions): void {
    ...
    const connectUrl = (id: string) => `${config.domain}${profilePath(id)}/mcp`;

    const withUrls = (id: string) => ({ url: connectUrl(id), setupUrl: `${config.domain}${profilePath(id)}` });
    ...
    api.patch('/profiles/:id', (req, res) => {
        try {
            ...
            if (req.body?.tallyPort !== undefined) patch.tallyPort = Number(req.body.tallyPort);
            if (req.body?.blockWrite !== undefined) patch.blockWrite = req.body.blockWrite === true;
