...
* Starting a session no longer starts a database. PGlite took about 1.7 seconds to start and it happened while the session was being registered, which is what a client waits on before it can call anything. Registering now takes a few milliseconds. A session that only reads a balance, sets a company or writes a master never starts one at all, and a session that runs a report pays the same cost a moment later, against its first report rather than against the handshake
* *query-database* is guarded like every other tool, so a syntax error or a table that has expired comes back as an error result instead of a transport failure
